...
for book in bookTitles:
    ...
    commission = clamp((price - json_sales['retailPrice']['amount']) / 100, 0.01, 0.5)
...
date_list = [today - datetime.timedelta(days = x+1)for x in range(1)] #range (1) days = 1 days ago
...
for cart_ID in range(1,cart_end):       #cart_ID is 1-9 apply as the numbers on function keys, applied in contain_ddl, shipping_ddl and order_ddl
    ...
    contain_ddl.append (f"INSERT INTO contains VALUES(\'{random.sample(isbns,1)[0]}\',{cart_ID},{random.randrange(1, quantity)});\n")
    shipping_ddl.append(f"INSERT INTO shipping VALUES({cart_ID}, \'{random.choice(fake_company_name)}\',\'{random.choice(fake_status)}\',{random.randint(1,100000)});\n")
    order_ddl.append(f"INSERT INTO order VALUES({cart_ID}, {date_list}, {random.choice(fake_addresses)},{bank_number}, {cart_ID},{cart_ID},{cart_ID}');\n")
for cart_ID in range(cart_end,cart_end+6):# empty carts for 5 fake users
    cart_ddl.append(f"INSERT INTO cart VALUES({cart_ID});\n")
#create users manually 
user_ddl = (f"INSERT INTO users VALUES(1, {cart_end+1}, 'esulu', 'pass', 'Eren Sulutas', 'eren@email.com', '123 Alexandria Way', false);\n"
            f"INSERT INTO users VALUES(2, {cart_end+2}, 'ben', '1234', 'ishappy', 'ben@email.com', '456 Admin Road', false);\n"
            f"INSERT INTO users VALUES(3, {cart_end+3}, 'josh', '1234', 'cl', 'josh@email.com', '763 Glendale Ave', false);\n"
            f"INSERT INTO users VALUES(4, {cart_end+4}, 'aaaa', '1234', 'AAA', 'AAA@email.com', '456 Admin Road', false);\n"
            f"INSERT INTO users VALUES(5, {cart_end+5}, 'bbbb', '1234', 'BBB', 'BBB@email.com', '868 Poor House Street', false);\n"
            )
